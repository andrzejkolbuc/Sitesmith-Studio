...
- [x] 4.1 Unit tests pass: `npm run test:unit` — c0ac500
- [x] 4.2 Integration tests pass: `npm run test:integration` — c0ac500
- [x] 4.3 Type checking passes: `npm run typecheck` — c0ac500
- [x] 4.4 Lint and format pass: `npm run check` — c0ac500
- [x] 4.5 Identical images produce zero changed pixels and no finding — c0ac500
- [x] 4.6 `comparability()`'s existing behaviour is unchanged, proven by its existing tests still passing untouched — c0ac500
- [x] 4.7 Two runs over the same snapshot pair produce identical regions — c0ac500
