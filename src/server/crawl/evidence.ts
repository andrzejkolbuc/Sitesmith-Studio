 ...
 *
 * **This module must import nothing.** It sits under `~/server` because the
 * `detail` shapes it decodes are produced next door in `findings.ts`, but it is
 * re-exported through the results view's `summarise.ts` and therefore reaches
 * the client bundle. An import from `findings.ts` or `crawler.ts` would pull the
 * crawler into the browser. The finding is taken structurally for that reason,
 * not by oversight.
 */
